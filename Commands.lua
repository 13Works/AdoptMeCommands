request = request or function(...)
        warn("[no function 'request']")
    end
getconnections = getconnections or function(...)
        warn("[no function 'getconnections']")
    end
isfile = isfile or function(...)
        warn("[no function 'isfile']")
    end
readfile = readfile or function(...)
        warn("[no function 'readfile']")
    end
writefile = writefile or function(...)
        warn("[no function 'writefile']")
    end
getupvalue = getupvalue or function(...)
        warn("[no function 'getupvalue']")
    end
...
function Click(Button)
    assert(
        ...
        "Arg 1 to function 'Click' must be a GuiButton object. Got: " ..
            (typeof(Button) == "Instance" and Button.ClassName or tostring(typeof(Button)))
    )
...
local function CompareKinds(Input, Source)
    ...
    local function FindMultipleSegments(InputSegment, PetName)
        ...
        for _, BaseSegment in BaseSegments do
            if
                (InputSegment:len() == 1 and BaseSegment:sub(1, 1) == InputSegment) or
                    (InputSegment:len() > 1 and InputSegment == BaseSegment)
             then
    ...
    local function FindSingleSegment(InputSegment, PetName)
        if string.find(PetName, InputSegment) or table.find(string.split(PetName, " "), InputSegment) then
    ...
    local InputSegments = Input:split("-")

    for _, Info in Source do
        local TotalSegmentsMatched = 0
        local PetName = string.lower(Info.name)
        if PetName == string.lower(Input) then
            return PetName
        end

        for _, InputSegment in ipairs(InputSegments) do
            InputSegment = Strip(string.lower(InputSegment))
            TotalSegmentsMatched =
                TotalSegmentsMatched +
                (#InputSegments > 1 and FindMultipleSegments(InputSegment, PetName) or
                    FindSingleSegment(InputSegment, PetName))
        end
...
local function GetTargets(Args)
    ...
    if Args.Sender ~= LocalPlayer and not HasAll(Args.Targets) then
        ...
        if not IsTarget then
            if testing then
                warn("Attempted to run command on non-target")
            end
            return
        end
    end
    ...
    assert(
        #Args.Targets <= Args.MaxTargets,
        string.gsub(
            "Too many targets were specified for the '*" ..
                Args.CommandName .. "' command. Max targets are: " .. Args.MaxTargets .. ". Got: " .. #Args.Targets,
            "*",
            Prefix
        )
    )
    ...
    local FoundTargets, NotFoundTargets = {}, {}

    for _, Target in Args.Targets do
        local TargetPlayer = nil

        for _, Player in Players:GetPlayers() do
            if string.lower(Player.Name) ~= string.lower(Target) and Player.UserId ~= tonumber(Target) then
                continue
            end
            TargetPlayer = Player
            break
        ...
        if not Accounts[TargetPlayer.Name] then
            warn(
                string.gsub(
                    TargetPlayer.Name ..
                        " must be in your 'Accounts' list. Do *Add_Account to add accounts to your 'Accounts' list.",
                    "*",
                    Prefix
                )
            )
        ...
        if TargetPlayer == Args.Sender and not Args.IncludeSender then
            continue
        end
        if table.find(FoundTargets, TargetPlayer) then
            continue
        end
    ...
    if #FoundTargets == 0 then
        warn("Player" .. (Args.MaxTargets > 1 and "s " or " ") .. "'" .. Concat(Args.Targets, "&") .. "' not found.")
    end

    return FoundTargets
end

local function ValidateFilters(Args)
    local Filters = Args
    ...
    if not Categories then
        if Properties then
            warn("Category/Categories must be provided to use 'Properties' filters.")
        end
        if Names then
            warn("Category/Categories must be provided to use 'Names' filters.")
        end
        if Amount then
            warn("Category/Categories & Name must be provided to use 'Amount' filter.")
        end
        if not Properties and not Names and not Amount then
            warn(
                (string.gsub(
                    "Category/Categories must be provided to use the '*" .. Args.CommandName .. "' command.",
                    "*",
                    Prefix
                ))
            )
...
local function GetFilters(Args)
    assert(Args.CommandName, "Missing argument 'CommandName'; Got: " .. tostring(Args.CommandName))
    Args.Filters = ValidateFilters(Args)
    if not Args.Filters then
        return
    end
    ...
    if vehicles then
        TargetCategories[vehicles] = "transport"
    end
...
local function PetListToDict(List)
    local Properties = {
        ["n"] = "neon",
        ["m"] = "mega_neon",
        ["f"] = "flyable",
    ...
    for _, Line in List:split("\n") do
        local TrimmedLine = Strip(Line)
        if testing then
            print("'" .. TrimmedLine .. "' |", typeof(TrimmedLine))
        end
        if TrimmedLine == "" then
            continue
        end

        local Amount, PetProperties, PetName = table.unpack(Line:split(" "))

        Amount = tonumber(Amount) or 0

        if PetName then
            PetProperties = PetProperties:split("")
        end

        if not PetName then
            PetName = Strip(PetProperties)
            PetProperties = {}
        end
        ...
        if next(PetProperties) ~= nil then
            ...
            print(PetProperties)
            for _, Property in PetProperties do
                if not Property then
                    continue
                end
                local FoundProperty = Properties[string.lower(Property)] or ""
                if FoundProperty == "" then
                    continue
                end
                PropertiesDict[FoundProperty] = true
            end

            PetProperties = PropertiesDict
        end

        local TargetKind, TargetName = CompareKinds(PetName)
        TargetKind = TargetKind or ""
        
        if testing then
            print("Amount:", Amount)
            print("InputName:", PetName)
            print(PetName, "(" .. TargetKind .. ")")
        end

        for Unique, Info in Inventory do
            if TargetKind == Info.kind then
                -- warn(TargetKind,"=", Info.kind, "| true")
            else
                -- print(TargetKind,"=", Info.kind, "| false")
                continue
            end
            ...
            if FoundPet then
                if Amount > 0 then
                    ...
                    warn("Found pet:", PetName, "(" .. TargetKind .. ")")
                    Uniques[Unique] = Info
                else
                    break
                end
    ...
    print("List To", TotalUniques, "Uniques:", Uniques)
    print("-------------------------------")
    return Uniques
end

local function ValidateItems(Args)
    local Properties = {
        newborn = "/a:1",
        reborn = "/a:1",
        junior = "/a:2",
    ...
    for Identity, Item_Uniques in GetIdentities(Args.Items) do
        if TargetAmount > 0 and Total == TargetAmount then
            break
        end
        local TargetItem = true

        local FoundProperties = string.split(PropertiesString, "/")
        if #FoundProperties == 1 then
            FoundProperties = {}
        end
        for _, Property in FoundProperties do
            if not string.find(Identity, "/" .. Property) then
                TargetItem = false
                break
            end
        end
...
local function GetDialogObject(ReturnButtons)
...
local function TradeTarget(Args)
    ...
    for i = 1, TotalItems, MaxTradeSize do
        ...
        repeat
            print("Checking if it's your first tiem trading.")
            task.wait(1)
        ...
        repeat
            ...
            task.wait(1)
        until (TradeFrame.NegotiationFrame.Visible == false and TradeFrame.ConfirmationFrame.Visible == true) or
            not GetCurrentTask(Args.Giver, "IsTrading")

        if not GetCurrentTask(Args.Giver, "IsTrading") then
            break
        end
...
function GetCurrentTask(Target, Task)
    assert(
        typeof(Target) == "Instance" and Target.ClassName == "Player",
        "Target must be a Player object. Got: " .. typeof(Target)
    )
    return Accounts[Target.Name][Task]
end

local function SetupTrade(Args)
    if LocalPlayer == Args.Reciever then
        if Args.Filters then
            print(
                "Getting every",
                Concat(Args.Filters.Properties or Args.Filters.Names, " & "),
                "from",
                Args.Giver.Name
            )
        end
        ...
        while GetCurrentTask(LocalPlayer, "IsTrading") do
            if not TradeFrame.Visible then
                ...
                repeat
                    ...
                    API:WaitForChild("TradeAPI/AcceptNegotiation"):FireServer()
                until (TradeFrame.NegotiationFrame.Visible == false and TradeFrame.ConfirmationFrame.Visible == true) or
                    not GetCurrentTask(Args.Reciever, "IsTrading")

                if not GetCurrentTask(Args.Reciever, "IsTrading") then
                    break
                end
        ...
        if Args.Filters and not Args.Items then
            ...
            for _, Category in Args.Filters.Categories do
                local ValidatedItems =
                    ValidateItems(
                    {["Items"] = Inventory[Category], ["Filters"] = Args.Filters, ["CommandName"] = Args.CommandName}
                )
...
local function SetupMultipleTrades(Args)
    local IsSender = (LocalPlayer == Args.Sender)
    PrintTable(Args.Targets, "Targets")
    for _, Target in Args.Targets do
        if IsSender then
            Say("Let's trade, " .. Target.Name .. ".")
        end
        repeat
            task.wait(1)
            print("Waiting for turn...")
        until GetCurrentTask(LocalPlayer, "IsTrading") or GetCurrentTask(Args.Sender, "CurrentTask") == "" or
            (IsSender and GetCurrentTask(Args.Sender, "IsTrading") == false)

        if
            GetCurrentTask(Target, "IsTrading") and GetCurrentTask(Args.Sender, "CurrentTask") ~= "" and
                (LocalPlayer == Target or IsSender)
         then
...
local function AcceptOrDeclineTrade(Args, AcceptOrDecline)
    Args.IncludeSender = true
    local Targets = GetTargets(Args)

    for _, Target in Targets do
        local CurrentTask = Accounts[Target.Name].CurrentTask
        if CurrentTask ~= "" then
            warn(
                LocalPlayer == Target and "You are" or Target.Name .. " is",
                "currently doing a task: '" ..
                    CurrentTask .. "'. Do '" .. Prefix .. Args.CommandName .. "' if you wish to cancel this task."
            )
            return
        end
        ...
        if IsTrading then
            ...
            warn(
                LocalPlayer == Target and "You have" or Target.Name .. " has",
                "no trade to " .. AcceptOrDecline and "accept" or "decline" .. ".",
                Args.Ignored
            )
        end
    end
...
function UpdateStatus(Target, Status, ItemsToReturn)
...
local function FormatInventoryToString(Inventory)
    ...
    for _, Pet in ipairs(Inventory) do
        totals.overall = totals.overall + Pet.amount
        totals.neons = totals.neons + (Pet.neon and Pet.amount or 0)
        totals.fullgrowns = totals.fullgrowns + (Pet.fullgrown and Pet.amount or 0)
    end
...
Commands["trade"] = function(Args) -- Trade item(s) to target
    local Targets = GetTargets(Args) or {}
    local MultipleTargets = #Targets > 1
    local Filters = GetFilters(Args)
...
Commands["add"] = function(Args) -- Add item(s) to current trade
    warn("The '" .. Prefix .. Args.CommandName .. "' command is currently disabled.")
end

Commands["get"] = function(Args) -- Get item(s) from targets
    local Targets = GetTargets(Args) or {}
    local MultipleTargets = #Targets > 1
    local Filters = GetFilters(Args)
...
Commands["distribute"] = function(Args) -- Distribute items to targets
    local Targets = GetTargets(Args) or {}
    local MultipleTargets = #Targets > 1
    local Filters = GetFilters(Args)
    local IsSender = (LocalPlayer == Args.Sender)
...
Commands["accept"] = function(Args)
    AcceptOrDeclineTrade(Args, true)
end

Commands["decline"] = function(Args)
    AcceptOrDeclineTrade(Args, false)
end

Commands["cancel"] = function(Args)
    Args.IncludeSender = true
    local Targets = GetTargets(Args)

    for _, Target in Targets do
        local CurrentTask = UpdateStatus(Target, nil, {"CurrentTask"})
        if CurrentTask and CurrentTask[1] == "" then
            warn(LocalPlayer == Target and "You have" or Target.Name .. " has", "no current task.", Args.Ignored)
            return
        end
...
Commands["add_alt"] = function(Args) -- Add alt(s) to alts list
    if Args.Sender ~= LocalPlayer then
        return
    end
    local Targets = GetTargets(Args)

    for _, Target in Targets do
        if Accounts[Target.Name] then
            warn(
                "Player '" ..
                    Target.Name ..
                        "' is already in your 'Accounts' list. Do '" ..
                            Prefix .. Args.CommandName .. "' to remove accounts."
            )
            continue
        end
...
Commands["remove_alt"] = function(Args) -- Remove alt(s) from alts list
    if Args.Sender ~= LocalPlayer then
        return
    end
    local Targets = GetTargets(Args)
    for _, Target in Targets do
        if not Accounts[Target.Name] then
            continue
        end
...
Commands["change_seperators"] = function(Args) -- Change current arguement seperators
    if Args.Sender ~= LocalPlayer then
        return
    end
    if not Args.Opening and not Args.Closing then
        warn("No opening or closing seperator specified.", Args.Ignored)
        return
    end
    ...
    if Opening == Closing then
        warn("Invalid seperators. You can not have the same opening/closing seperator. |", ArgumentSeperators.Opening)
        return
    end
    if not Opening:match("%W") then
        warn("Opening separator is not a special character.", Args.Ignored)
        return
    end
    if not Closing:match("%W") then
        warn("Closing separator is not a special character.", Args.Ignored)
        return
    end
    if #string.split(Opening, "") > 1 then
        warn("Opening separator must be a single special character.", Args.Ignored)
        return
    end
    if #string.split(Closing, "") > 1 then
        warn("Closing separator must be a single special character.", Args.Ignored)
        return
    end
...
Commands["change_prefix"] = function(Args) -- Change current prefix
    if Args.Sender ~= LocalPlayer then
        return
    end
    ...
    if not NewPrefix then
        warn("No prefix specified.", Args.Ignored)
        return
    end
    if #NewPrefix > 1 then
        warn("Only 1 character is required for the '" .. Prefix .. Args.CommandName .. "' command.")
        return
    end

    NewPrefix = NewPrefix[1]

    if table.find(BlacklistedCharacters, NewPrefix) then
        warn("Cannot set prefix to blacklisted character: '" .. NewPrefix .. "'.", Args.Ignored)
        return
    end
    if not NewPrefix:match("%W") then
        warn("Given prefix is not a special character.", Args.Ignored)
        return
    end
...
Commands["inventory"] = function(Args) -- Send inventory through webhook
    if Args.Sender ~= LocalPlayer then
        return
    end
    if Webhook == "" then
        warn("You never entered your webhook url.", Prefix .. Args.CommandName, "ignored.")
        return
    end
    local Filters = GetFilters(Args)
    local Inventory = GetInventory()
    local TargetItems, Items = {}, {}
    for _, Unique in ValidateItems({["Items"] = Inventory, ["Filters"] = Filters}) do
        table.insert(TargetItems, Unique)
    end
    ...
    for Unique, Info in Inventory.pets do
        if not table.find(TargetItems, Unique) then
            ...
            if Items[Info.kind] then
                Items[Info.kind].amount = Items[Info.kind].amount + 1
                continue
            end
            local Item = {}
            Item.name = Info.kind
...
Commands["help"] = function(Args) -- Standard help command
    ...
    if not Args.command or not Args.commands then
        for CommandName, _ in CommandMessages do
            print(Prefix .. CommandName)
        end
        return
    end
    local TargetCommands = Args.commands or Args.command or {}
    if next(TargetCommands) == nil then
        return
    end
    ...
    if table.find(TargetCommands, "all") or table.find(TargetCommands, "All") then
        TargetCommands = {}
        for CommandName, _ in CommandMessages do
            table.insert(TargetCommands, CommandName)
        end
    end

    for _, TargetCommand in TargetCommands do
        TargetCommand = (string.gsub(string.lower(TargetCommand), Prefix, ""))
        if not CommandMessages[TargetCommand] then
            warn("Command '" .. TargetCommand .. "' not found.")
            continue
        end
...
local function ValidateCommand(Message, Sender)
    local Opening =
        table.find(SpecialCharacters, ArgumentSeperators.Opening) and "%" .. ArgumentSeperators.Opening or
        ArgumentSeperators.Opening
    local Closing =
        table.find(SpecialCharacters, ArgumentSeperators.Closing) and "%" .. ArgumentSeperators.Closing or
        ArgumentSeperators.Closing
...
function Say(Message, IsSystemMessage)
    local Channel = IsSystemMessage and "RBXSystem" or "RBXGeneral"
    local TextChannel = TextChatService.TextChannels[Channel]
...
local Connection =
    ...
    function(Message)
        if not Message or not Message.TextSource then
            error("Error processing message. Got:" .. typeof(Message))
        end
...
Players.PlayerRemoving:Connect(
    function(Player)
        if not Accounts[Player.Name] then
            return
        end
        UpdateStatus(Player)
    end
