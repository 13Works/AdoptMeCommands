...
function Click(Button: GuiButton)
	assert(typeof(Button) == "Instance" and Button:IsA("GuiButton"), "Arg 1 to function 'Click' must be a GuiButton object. Got: ".. (typeof(Button) == "Instance" and Button.ClassName or tostring(typeof(Button))))
...
local function CompareKinds(Input: string, Source: {{["name"]: string}})
	...
	for _, Info in Source do
		...
		if PetName == string.lower(Input) then return PetName end
...
local function PetListToDict(List: string)
	local Properties = {
		["n"] = "neon",
		["m"] = "mega_neon",
		["f"] = "flyable",
	...
	for _, Line in List:split("\n") do
		...
		if testing then print("'".. TrimmedLine .."' |", typeof(TrimmedLine)) end
		if TrimmedLine == "" then continue end
		...
		if next(PetProperties) ~= nil then
			...
			for _, Property in PetProperties do
				...
				if FoundProperty == "" then continue end
		...
		for Unique, Info in Inventory do
			...
			for Property, Value in PetProperties do
				if Info.properties[Property] == nil then FoundPet = false continue end
				if Info.properties[Property] ~= Value then FoundPet = false continue end
			end
